...
template<class T>
ListCTKH<T>::ListCTKH(string _tenTacGia) : soLuongCTKH(0), tenTacGia(_tenTacGia), data(nullptr) {}
template<class T>
ListCTKH<T>::~ListCTKH()
{
    delete[] this->data;
...
template<class T>
void ListCTKH<T>::operator=(const ListCTKH& other)
{
    this->soLuongCTKH =other.soLuongCTKH;
    this->tenTacGia = other.tenTacGia;
    this->data= new T*[this->soLuongCTKH];
    for(int i=0;i<other.soLuongCTKH;++i)
    {
        this->data[i] = other.data[i];
    }
}
template<class T>
void ListCTKH<T>::Add(T *CTKH)
{
    if(this->soLuongCTKH ==0)
    {
        this->data = new T*[this->soLuongCTKH+1];
        this->data[this->soLuongCTKH] = CTKH;
    }
    else
    {
    ...
    if(BaiBao* bb = dynamic_cast<BaiBao*>(data[index]))
    {
        ...
        while (dung ==false)
        {
            ...
            case 4: 
            {
                ...
                this->data[index]->setPublisherYear(namxb);
                this->data[index]->Show();
                cout<<endl;
                break;
            }
