...
{   
    ...
    new_id = get_number_from_string(current->data.getOrder_id()) + 1;
    string temp = "HD" + to_string(new_id);
    this->order_id = temp;
    int chon;
    getchar();
    ...
    else
    {
        ...
        if (chon == 'Y' || chon == 'y')
        {
            ...
            this->sdt_cus = current->data.get_phone_num();
            return 1;
        }
...
{   
    ...
    for (int i = 0; i < so_loai; i++)
    {
        ...
        Node_Book *current = headBook;
        while(current!=nullptr) {
            if(current->data.get_id() == book_sold[i].get_book_id()) {
    ...
    s.setEmp_id(id_emp);
    Node_Save *newNode = new Node_Save(s);
    if (headCus == nullptr)
    {
        headSave = newNode;
    }
    ...
    if (daco == 1)
    {
        ...
        while(current != nullptr)
        {
            if (current->data.get_phone_num() == sdt_cus)
            {
                if (current->data.get_point() >= 50)
                {
                    discount = sum * 0.05;
                }
                current->data.re_point(sum / 100000);
                break;
            }
            current=current->next;
        }
...
void order::decrease_store()
{
    ...
    while(current !=nullptr)
    {
        int qtt_old = current->data.get_quantity();
        int qtt_new = qtt_old - solds[current->data.get_id()];
        current->data.set_quantity(qtt_new);
    }
}
